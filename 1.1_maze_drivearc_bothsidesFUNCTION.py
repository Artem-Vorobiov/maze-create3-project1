...
from irobot_create_msgs.action import DriveArc, RotateAngle, WallFollow, NavigateToPosition
...
class listener(Node):
	...
	# EVERY 2 SECOND THIS CALLBACK UPDATES
	def timer_callback(self):
		if self.ir_readings: 
			...
			print("ITERATION")
			print("")
			print(self.tf_coords.translation.y)				# prints our the currant Y coordinate
			if self.tf_coords.translation.y < 1.3:			# if coordinate Y less than 1.3 follow the instructions
				self.side = 1 								# follow the wall LEFT side
				self.radians = 2.0 							# make the left side arch 
				self.sensor_0_or_6 = 0 						# the first sensor on the left side of the robot
				self.sensor_1_or_5 = 1 						# the second sensor on the left side of the robot
				self.command_panel()						# run the main logic
			else:
				self.side = -1    							# follow the wall RIGHT side
				self.radians = - 2.0 						# make the right side arch 
				self.sensor_0_or_6 = 5  					# the sixth sensor
				self.sensor_1_or_5 = 6 						# the seventh sensor
				self.command_panel()						# run the main logic
	...
	def command_panel(self):			
		if len(self.hazard_list) == 1 and self.state == "Navigation":	# If bump wall, change state to wall follow
			...
			print(self.hazard_list[self.sensor_0_or_6].type)								# if bump then shows 1
			print(self.hazard_list[self.sensor_0_or_6].header.frame_id)						# if bump shows the side that is bumped
			print(str(self.ir_readings[self.sensor_0_or_6].value) + " Zero")
			print(str(self.ir_readings[self.sensor_1_or_5].value) + " First")				# prints values of ir sensors 0 and 1
			print(self.state)
		...
		elif (self.ir_readings[self.sensor_0_or_6].value > 50 or self.ir_readings[self.sensor_1_or_5].value > 75) and self.state == "WallFollow":
			# these conditions maintain the robot to follow the wall.
			print(str(self.ir_readings[self.sensor_0_or_6].value) + " Zero")
			print(str(self.ir_readings[self.sensor_1_or_5].value) + " First")
		...
		elif self.ir_readings[self.sensor_0_or_6].value < 50 and self.ir_readings[self.sensor_1_or_5].value < 70 and self.state == "WallFollow":
	...
	def send_goal_arc(self):
		...
		move.angle = self.radians # radians
